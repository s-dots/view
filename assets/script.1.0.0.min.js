...
$(function() {
	...
	$("#alarm-popup li").click(function() {
		...
		if (pinToArm && (saCommand == "armaway" || saCommand == "armastay") || pinToDisarm && saCommand == "disarm") {
			//alert(0);
			//alert($("#pincode-popup").size());
			$("#pincode-popup").popup("open");
			setTimeout( function() { $("#pincode-popup").popup( 'open' ); }, 400 );
			//alert(2);
		} else {
	...
	$("#pincode-popup button").click(function() {
...
function sendCommand(type, device, command, value) {
	...
	$.get("command", request).done(function( data ) {
		...
		if (data.status == "ok") {
			nextPoll(5);
		} else if (data.status == "saFail") {
			nextPoll(5);
			alert(data.message + "\n" + "Check SmartAlarm configuration");
		}
	}).fail(function() {
		setWTFCloud();
		nextPoll(10);
...
function updateTile(data) {
	if (data.tile == "device") {
		...
		var tile = $(".alarm");
		if (data.status != tile.attr("data-status")) spinner(tile);
		tile.attr("data-status", data.status);
		tile.find(".footer").html(data.status);
	}
}
