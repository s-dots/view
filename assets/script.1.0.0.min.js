...
$(function() {
	...
	$(".mode, .hello-home, .thermostat, .alarm").click(function() {
	...
	$("#alarm-popup li").click(function() {
		...
		var pinToArm = $(".alarm").data("pin-to-arm");
		var pinToDisarm = $(".alarm").data("pin-to-disarm");
		saCommand = $(this).data("state");
		
		if (pinToArm && (saCommand == "armaway" || saCommand == "armstay") || pinToDisarm && saCommand == "disarm") {
			$("#pincode-popup").popup("open");
			setTimeout( function() { $("#pincode-popup").popup( 'open' ); }, 200 );
		} else {
			sendCommand("alarm", null, saCommand);
		}
    });
	
	$("#pincode-popup a").click(function() {
		if ($(this).data("value") == "cancel") {
			$("#pincode").val("");
			$("#pincode-popup").popup("close");
		} else if ($(this).data("value") == "back") {
			$("#pincode").val(($("#pincode").val() + "").slice(0, - 1));
		} else if ($(this).data("value") == "enter") {
			$("#pincode-popup").popup("close");
			sendCommand("alarm", null, saCommand, $("#pincode").val());
			$("#pincode").val("");
		} else {
			$("#pincode").val($("#pincode").val() + "" + $(this).data("value"));
		}
	});
...
function showAlarmConfimration(status, success) {
	$("#alarm-confirmation-popup h4").hide();
	$("#alarm-confirmation-popup h4 span").html(status);
	//.html('<i class="fa ' + $(this).data("icon") + '"></i> ' + status);
	
	if (success) {
		$("#alarm-confirmation-popup h4.st-sa-success").show();
	} else {
		$("#alarm-confirmation-popup h4.st-sa-fail").show();
	}
	
	setTimeout( function() { $("#alarm-confirmation-popup").popup( 'open' ); }, 200 );
	setTimeout( function() { $("#alarm-confirmation-popup").popup( 'close' ); }, 7000 );
}
	...
	$.get("command", request).done(function( data ) {
		...
		if (data.status == "ok") {
			...
			if (data.success) {
				...
				showAlarmConfimration(data.status, data.success);
			} else {
				showAlarmConfimration("access denied", data.success);
			}
		}
	}).fail(function() {
		setWTFCloud();
...
function updateAlarmTile(status) {
	var isActive = (status == "disarmed" ? "inactive" : "active");
	var tile = $(".alarm");
	if (status != tile.attr("data-status")) spinner(tile);
	tile.attr("data-status", status);
	tile.find(".footer").html(status);
